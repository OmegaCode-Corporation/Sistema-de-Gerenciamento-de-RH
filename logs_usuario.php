...
<body>
    ...
    <div class="card card-body mt-5 container" id="cardbox">
        <center><h1>Histórico de Usuário</h1></center>
        ...
        <?php 
            ...
            if(isset($_GET['pesquisa']) && !empty($_GET['pesquisa'])){
                ...
                $total_pages = ceil($class_db->fetchData("SELECT COUNT(*) AS total FROM historico_usuarios WHERE id_historico = '$pesquisa'")[0]['total'] / $registros_por_pagina);
                $historicos = $class_historico->listarHistoricos(null, $pesquisa, $offset, $registros_por_pagina);
            } else {
                $total_pages = ceil($class_db->fetchData("SELECT COUNT(*) AS total FROM historico_usuarios")[0]['total'] / $registros_por_pagina);
        ...
        <?php
                        foreach($historicos as $historico){
                            ...
                            $movimentacao = ucwords(strtolower($historico['movimentacao']));
                ...
                <nav aria-label="Paginação">
                    <ul class="pagination justify-content-center">
                        ...
                        <li class="page-item <?php echo $page <= 1 ? 'disabled' : ''; ?>">
                            <a class="page-link" href="<?php echo $page <= 1 ? '#' : './logs_usuario.php?page=' . ($page - 1) . $pesquisa; ?>" tabindex="-1" aria-disabled="true"><<</a>
                        </li>
                        <?php for ($i = 1; $i <= $total_pages; $i++) : ?>
                            <li class="page-item <?php echo $page == $i ? 'active' : ''; ?>">
                                <a class="page-link" href="./logs_usuario.php?page=<?php echo $i . $pesquisa; ?>"><?php echo $i; ?></a>
                            </li>
                        <?php endfor; ?>
                        <li class="page-item <?php echo $page >= $total_pages ? 'disabled' : ''; ?>">
                            <a class="page-link" href="<?php echo $page >= $total_pages ? '#' : './logs_usuario.php?page=' . ($page + 1) . $pesquisa; ?>">>></a>
                        </li>
                    </ul>
                </nav>
                </center>

...
<head>
    ...
    <title>Histórico de Sanções</title>
    <link rel="stylesheet" href="assets/css/logs_sancoes.css">
...
<body>
    ...
    <div class="card card-body mt-5 container" id="cardbox">
        <center><h1>Histórico de Sanções</h1></center>
            ...
            <div class="flex" style="display: flex;">
                ...
                <div class="dropdown" id="searchInfo"><i class="bi bi-info-circle" style="font-size: 26px;"></i>
                    <div class="dropdown-content" id="searchInfo_content">
                        <p> <p>Opções de Procura:</p><b>ID Sanção;</b> <br> </p>
                    </div>
                </div>
            </div>
            <div class="row">
        ...
        <?php 
            ...
            if(isset($_GET['pesquisa']) && !empty($_GET['pesquisa'])){
                ...
                $total_pages = ceil($class_db->fetchData("SELECT COUNT(*) AS total FROM historico_sancoes WHERE id_historico = '$pesquisa'")[0]['total'] / $registros_por_pagina);
                $historicos = $class_historico->listarHistoricos(null, $pesquisa, $offset, $registros_por_pagina);
            } else {
                $total_pages = ceil($class_db->fetchData("SELECT COUNT(*) AS total FROM historico_sancoes")[0]['total'] / $registros_por_pagina);
        ...
        <?php
                        foreach($historicos as $historico){
                            ...
                            $tipo_sancao = ucwords(strtolower($historico['tipo_sancao']));
                            ...
                            echo "<tr>
                                ...
                                <td>
                                    <div class='btns' style='display: flex; flex-direction: row'>
                                        <button class='btn' data-toggle='modal' data-target='#viewModal__viewModal__$id_historico' alt='Ver Mais'><i class='bi bi-journal-text'></i></button>
                                    </div>
                ...
                <nav aria-label="Paginação">
                    <ul class="pagination justify-content-center">
                        ...
                        <li class="page-item <?php echo $page <= 1 ? 'disabled' : ''; ?>">
                            <a class="page-link" href="<?php echo $page <= 1 ? '#' : './logs_sancoes.php?page=' . ($page - 1) . $pesquisa; ?>" tabindex="-1" aria-disabled="true"><<</a>
                        </li>
                        <?php for ($i = 1; $i <= $total_pages; $i++) : ?>
                            <li class="page-item <?php echo $page == $i ? 'active' : ''; ?>">
                                <a class="page-link" href="./logs_sancoes.php?page=<?php echo $i . $pesquisa; ?>"><?php echo $i; ?></a>
                            </li>
                        <?php endfor; ?>
                        <li class="page-item <?php echo $page >= $total_pages ? 'disabled' : ''; ?>">
                            <a class="page-link" href="<?php echo $page >= $total_pages ? '#' : './logs_sancoes.php?page=' . ($page + 1) . $pesquisa; ?>">>></a>
                        </li>
                    </ul>
                </nav>
                </center>

...
<head>
    ...
    <title>Histórico de Acidentes de Trabalho</title>
    <link rel="stylesheet" href="assets/css/logs_acidentes_trabalho.css">
...
<body>
    ...
    <div class="card card-body mt-5 container" id="cardbox">
        <center><h1>Histórico de Acidentes de Trabalho</h1></center>
            ...
            <div class="flex" style="display: flex;">
                ...
                <div class="dropdown" id="searchInfo"><i class="bi bi-info-circle" style="font-size: 26px;"></i>
                    <div class="dropdown-content" id="searchInfo_content">
                        <p> <p>Opções de Procura:</p><b>ID Acidente;</b> <br> </p>
                    </div>
                </div>
            </div>
            <div class="row">
        ...
        <?php 
            ...
            if(isset($_GET['pesquisa']) && !empty($_GET['pesquisa'])){
                ...
                $total_pages = ceil($class_db->fetchData("SELECT COUNT(*) AS total FROM historico_acidentes_trabalho WHERE id_historico = '$pesquisa'")[0]['total'] / $registros_por_pagina);
                $historicos = $class_historico->listarHistoricos(null, $pesquisa, $offset, $registros_por_pagina);
            } else {
                $total_pages = ceil($class_db->fetchData("SELECT COUNT(*) AS total FROM historico_acidentes_trabalho")[0]['total'] / $registros_por_pagina);
        ...
        <?php
                foreach($historicos as $historico){
                    ...
                    $id_colaborador = $historico['id_colaborador'];
                    $id_usuarioResp = $historico['id_usuario'];
                    ...
                    $dadosColaboradorResp = $class_usuario->listarUsuarios($id_usuarioResp)[0];
                    $nome_colaboradorResp = ucwords(strtolower($class_usuario->listarColaboradores($dadosColaboradorResp['id_colaborador'])[0]['nome_colaborador']));
                    
                    $movimentacao = ucwords(strtolower($historico['descricao']));
                    $movimentacao_fstLine = str_contains($movimentacao, "<br>") ? substr($movimentacao, 0, strpos($movimentacao, "<br>")) : $movimentacao;

                    $afastamento_inss = $historico['afastamento_inss'] === 1 ? "Sim" : "Não";

                    $data_inicio = date("d/m/Y", strtotime($historico['data_inicio']));
                    $data_termino = date("d/m/Y", strtotime($historico['data_termino']));
                    $data_movimentacao = date("d/m/Y H:i:s", strtotime($historico['data_movimentacao']));

                    echo "<tr>
                        <td>$id_historico</td>
                        <td>$nome_colaborador</td>
                        <td>$nome_colaboradorResp</td>
                        <td>$afastamento_inss</td>
                        <td>$data_inicio</td>
                        <td>$data_termino</td>
                        <td>$data_movimentacao</td>
                        <td>
                            <div class='btns' style='display: flex; flex-direction: row'>
                                <button class='btn' data-toggle='modal' data-target='#viewModal__$id_historico' alt='Ver Mais'><i class='bi bi-journal-text'></i></button>
                            </div>
                            <div class='modal fade' id='viewModal__$id_historico' tabindex='-1' role='dialog' aria-labelledby='viewModalLabel' aria-hidden='true'>
                                <div class='modal-dialog modal-dialog-centered' role='document'>
                                    <div class='modal-content'>
                                        <div class='modal-header'>
                                            <h4 class='modal-title' id='viewModalLabel__$id_historico'>Visualizar Histórico</h4>
                                            <button type='button' class='close' data-dismiss='modal' aria-label='Close'>
                                                <span aria-hidden='true'>&times;</span>
                                            </button>
                                        </div>
                                        <div class='modal-body'>
                                            <textarea class='form-control' style='height:125px' readonly>$movimentacao</textarea>
                                        </div>
                ...
                <nav aria-label="Paginação">
                    <ul class="pagination justify-content-center">
                        ...
                        <li class="page-item <?php echo $page <= 1 ? 'disabled' : ''; ?>">
                            <a class="page-link" href="<?php echo $page <= 1 ? '#' : './logs_acidentes_trabalho.php?page=' . ($page - 1) . $pesquisa; ?>" tabindex="-1" aria-disabled="true"><<</a>
                        </li>
                        <?php for ($i = 1; $i <= $total_pages; $i++) : ?>
                            <li class="page-item <?php echo $page == $i ? 'active' : ''; ?>">
                                <a class="page-link" href="./logs_acidentes_trabalho.php?page=<?php echo $i . $pesquisa; ?>"><?php echo $i; ?></a>
                            </li>
                        <?php endfor; ?>
                        <li class="page-item <?php echo $page >= $total_pages ? 'disabled' : ''; ?>">
                            <a class="page-link" href="<?php echo $page >= $total_pages ? '#' : './logs_acidentes_trabalho.php?page=' . ($page + 1) . $pesquisa; ?>">>></a>
                        </li>
                    </ul>
                </nav>
                </center>
